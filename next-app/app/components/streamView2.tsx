...
export default function StreamView2({
  ...
  const [message,setMessages] = useState('MT')
  ...
  const updateStreams = async (messageData: any) => {
    const json = messageData;
    setQueue(json.streams.sort((a: any, b: any) => a.upvotes < b.upvotes ? 1 : -1));

    // Update the current video only if it's not already active
    setCurrentVideo((video) => {
        if (video?.id === json.activeStream?.stream?.id) {
            return video;
        }
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event: any) => {    
        console.log("on mesage in client , refresh stream called")  
        console.log(event)  
        refreshStreams();
    };
    ws.onerror=()=>{
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-5 w-screen max-w-screen-xl pt-8">
                <div className='col-span-3'>
                    <div className="space-y-4">
                        <h2 className="text-2xl font-bold text-white">Upcoming Songs {message}</h2>
                ...
                <div className='col-span-2'>
                    <div className="max-w-4xl mx-auto p-4 space-y-6 w-full">
                        ...
                        />
                        <Button disabled={loading} onClick={(e)=>{handleSubmit(e) }} type="submit" className="w-full bg-purple-700 hover:bg-purple-800 text-white">{loading ? "Loading..." : "Add to Queue"}</Button>
                        </form>

                        {inputLink && YT_REGEX.test(inputLink) && !loading && (
